...
namespace NorthwindInventory.NorthwindDB
{
    public class ProductService
    {
        ...
        {
            _dbService = new DbService<DbProject>(false, configuration);
        ...
        public async Task<Dictionary<string,Category>> GetCategoriesAsync()
        {
            return await LoadReferenceData<Category>(_categoryTableClient);
        }

        public async Task<Dictionary<string, Supplier>> GetSuppliersAsync()
        {
            return await LoadReferenceData<Supplier>(_supplierTableClient);
        }
        ...
        private IProductEx GetProductExForEntity(TableEntity entity)
        {
            try
            {
                ...
                productEx.AverageDiscount = _orderTotals != null && _orderTotals.ContainsKey(productEx.ProductID.ToString())? Math.Round(productEx.Revenue / _orderTotals[productEx.ProductID.ToString()].TotalDiscount, 1): 0;
        ...
        public async Task<string> CreateProductAsync(Product product)
        {
            ...
            var nextId = await _dbService.GetNextIdAsync(TableNames.Product);

            // Create the new product entity
            var newProduct = new DbProject
            {
                PartitionKey = TableNames.Product,
            ...
            await _dbService.CreateEntityAsync(TableNames.Product, newProduct.RowKey, newProduct);
